...
pub fn format_metadata(detail: &WorkflowDetail) -> Vec<(String, String)> {
    vec![
        ...
        ("Type".to_string(), detail.summary.workflow_type.to_string()),
        ("Status".to_string(), format!("{}", detail.summary.status)),
        ("Task Queue".to_string(), detail.summary.task_queue.to_string()),
        (
            "Started".to_string(),
    ...
    fn make_detail() -> WorkflowDetail {
        WorkflowDetail {
            summary: WorkflowSummary {
                ...
                workflow_type: std::sync::Arc::from("TestWorkflow"),
